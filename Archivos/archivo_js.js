...
(function() {
    ...
    var cipher_text = document.getElementsByClassName("AES")[0].id;
    var nonce = document.getElementsByClassName("iv")[0].id;


    //key =   CryptoJS.enc.Base64.parse(key)
    nonce = CryptoJS.enc.Base64.parse(nonce)
    console.log('LLave : '+ key)
    console.log('Nonce : '+ nonce)

    var decrypt = CryptoJS.AES.decrypt(cipher_text,key,{nonce: nonce}, CryptoJS.mode.EAX,
    ...
    var recoveredPlaintext = eax.decrypt(cipher_text, nonce);
    console.log(recoveredPlaintext.toString())
    ...
    t = document.createTextNode(decrypt.toString());
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);

    var input_key = 'sinla_contrasena';
    var input_msg = 'hola';
    var input_nonce = CryptoJS.lib.WordArray.random(16);

     input_key = CryptoJS.enc.Hex.parse(input_key)
     var input_eax = CryptoJS.EAX.create(input_key);

     var ciphertext = input_eax.encrypt(input_msg, input_nonce);
     var input_recoveredPlaintext = input_eax.decrypt(ciphertext, input_nonce);
     ...
     z = document.createElement("TD");
     t = document.createTextNode(input_recoveredPlaintext.toString(CryptoJS.enc.Utf8));
     z.appendChild(t);
     document.getElementById("Tr").appendChild(z);
